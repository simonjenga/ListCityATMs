...
To view all the ATMs that are listed by this web application, open the URL: ```http://localhost:8080/ListCityATM/listATMs``` in a web browser.

To view only the ATMs that are listed for a particular city open the URL: ```http://localhost:8080/ListCityATM/listATMsByCity?city=CITY_NAME``` in a web browser and then
substitute the parameter value with the actual city name e.g ```MAASTRICHT``` , ```AMSTERDAM``` , ```ROTTERDAM``` , ```EINDHOVEN``` , ```MIDDELBURG```, ```ZOETERMEER``` e.t.c

To view the home page of this web application, open the URL: ```http://localhost:8080/ListCityATM/``` in a web browser.
